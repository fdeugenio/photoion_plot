...
def example_plot_ssp_metal_logu_interp(
    cmap='viridis', alpha=0.5,
    ):
    ...
    for i,subgrid in enumerate(hkt):
        ...
        for j,(_x_, _y_) in enumerate(zip(
            subgrid[0, 1:-1, 0, 1:-1, 5], subgrid[0, 1:-1, 0, 1:-1, 6])):
        ...
        for j,(_x_, _y_) in enumerate(
            zip(subgrid[0, 1:-1, 0, 1:-1, 5].T, subgrid[0, 1:-1, 0, 1:-1, 6].T)):
...
def example_plot_ssp_metal_logu_interp_roberta(
    ...
    hkt_unpadded = read_harkatz_models(
        ...
        param2='Ne 3 3868.76A/O  2 3727A',    # [NeIII]3868/[OII]3727
        downsample={'ssp type': (0), 'age': 5, 'logn': (1.5, 2, 2.5),
            'metal': (0.001, 0.003, 0.006, 0.014, 0.03),
            'logU': (-4., -3., -2., -1., -0.5)
            }
        )
    ...
    for i,subgrid in enumerate(hkt):
        ...
        logn = subgrid[0, 1, 0, 1, 3] # Ones because of cladding...
...
if __name__=="__main__":
    example_plot_ssp_metal_logu_interp()
